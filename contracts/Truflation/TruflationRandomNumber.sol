...
contract TruflationRandomNumber is ChainlinkClient {
    ...
    function randomNumberRequestChainlinkTruflation() public returns (bytes32 requestId) {
        Chainlink.Request memory req = buildChainlinkRequest(bytes32(bytes(jobId)), address(this), this.fulfillBytes.selector);
        req.add("service", "truflation/data/test");
        // req.add("data", "{'id':'8002050'}"); //Crude Oil (price server API is down right now.
